...
class Fale_conosco:
    ...
    def inserindo_etag(self, etag, objeto):
        novo_campo = OrderedDict([('Etag', etag)])
        objeto.update(novo_campo)
   
    def inserindo_partition_key_fale_conosco(self, partition_key, objeto):
        novo_campo = OrderedDict([('PartitionKey', partition_key)])
        objeto.update(novo_campo)
      
    def verificar_entities_fale_conosco(self, lista_sql, lista_azurite, partition_key):
        ...
        if not lista_azurite:
            for elemento in lista_sql:
                self.inserindo_partition_key_fale_conosco(partition_key, elemento)
                ...
                for elemento_azurite in lista_azurite:
                    if elemento_sql['RowKey'] == elemento_azurite['RowKey']:
                        if elemento_sql['Categoria'] == elemento_azurite['Categoria'] and elemento_sql['Ativo'] == elemento_azurite['Ativo']:
                            #verfico se no elemento da azureList tem ETag
                            if 'Etag' in elemento_azurite:
                                pass
                            else:
            ...
            for elemento in lista_para_enviar_azure:
                self.inserindo_etag(elemento['Ativo'], elemento)
                self.inserindo_partition_key_fale_conosco(partition_key, elemento)
    ...
    def atualiza_tabela_fale_conosco(self):
        partition_key = 'c124bab6-7011-4cfb-8ca9-4934b45e30f5'
        ...
        lista_para_enviar_azure = self.verificar_entities_fale_conosco(lista_sql, lista_azurite, partition_key)    
        self.enviando_azure(lista_para_enviar_azure, table_client)