...
class Sub_Duvida:
    ...
    def conexao_sql_duvida(self, str_sql): # faz a conexão busca os dados e retorna um lista de objetos
        try:
            ...
            query_sql = """
                    SELECT  
                        [idDocumenta] As RowKey
                        ,[Tipo] as Duvida
                        ,[Titulo]
                        ,[Texto]
                        ,[Ativo]
                    FROM 
                        [RECEITA].[dbo].[DOCUMENTAWEB]
                    where 
                        Tipo in (SELECT distinct   [cdtipo]      
                    FROM 
                        [RECEITA].[dbo].[TIPOSERVICO] as T 
                    inner join 
                        [RECEITA].[dbo].[DOCUMENTAWEB] as D on T.cdtipo = D.Tipo 
                    WHERE d.Ativo = 'S' and T.documenta = 'S') and Ativo = 'S'
                    """
    ...
    def inserindo_sub_duvida_partition_key(self, partition_key, objeto):
        novo_campo = OrderedDict([('DuvidaPartitionKey', partition_key)])
        objeto.update(novo_campo)
    ...
    def verificar_entities(self, lista_sql, lista_azurite, partition_key_duvida, partition_key_sub_duvida):
        ...
        if not lista_azurite:
            for elemento in lista_sql:
                self.inserindo_partition_key(partition_key_duvida, elemento)
                self.inserir_etag(elemento['Ativo'], elemento)
                self.inserindo_sub_duvida_partition_key(partition_key_sub_duvida, elemento)
            ...
            for elemento_sql in lista_sql:
                for elemento_azurite in lista_azurite:
                    if elemento_sql['RowKey'] == elemento_azurite['RowKey']:
                        if elemento_sql['Duvida'] == elemento_azurite['Duvida'] and elemento_sql['Titulo'] == elemento_azurite['Titulo'] and elemento_sql['Titulo'] == elemento_azurite['Titulo'] and elemento_sql['Texto'] == elemento_azurite['Texto'] and elemento_sql['Ativo'] == elemento_azurite['Ativo']:
                        ...
                        #Caso row key n exista devo montar a entities
                        inserir = True
    ...
    def atualiza_tabela_sub_duvida(self):
        partition_key_sub_duvida = "315398a0-32d2-4839-8dcc-1da8afa7e76d"
        duvida_partition_key = "74f40075-1d24-45ef-b984-b9e58e3332c7"
        ...
        lista_para_inserir = self.verificar_entities(lista_sql, lista_azurite, duvida_partition_key, partition_key_sub_duvida)   
